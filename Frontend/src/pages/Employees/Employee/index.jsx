...
const Employee = ({ employee }) => {
  const { handleEditModalOpen, deleteEmployee } = useContext(EmployeeContext);
    ...
    if (directSubordinates.length === 0) {
      try {
        const response = await axiosInstance.get(
          "/allemployeesUnderManager/" + employee.employeeId
        );
  ...
  return (
    <div>
      <div className="flex flex-row items-center gap-x-2">
        {employee.count > 0 && (
          <div
            className="bg-slate-300 px-1 py-1 cursor-pointer"
            onClick={expanded ? handleMinusClick : handlePlusClick}
          >
        ...
        <div
          className="cursor-pointer bg-slate-300 p-1"
          onClick={() => handleEditModalOpen(employee.employeeId)}
        >
          <MdEdit />
        </div>
      </div>
